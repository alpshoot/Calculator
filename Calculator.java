...
public class Calculator {
    ...
    public static String calc(String input) throws Exception {
        ...
        String[] operands = input.split("[+\\-*/]"); //операнды
        ...
        operation = OperationTest(input);

        //если числа римские
        if(Roman.thisRom(operands[0])&& Roman.thisRom(operands[1])){                       
        ...
        //если числа арабские
        else if (!Roman.thisRom(operands[0])&& !Roman.thisRom(operands[1])){
        ...
        int arab = calculation(number1, number2, operation);
        if(romanNumber){
